...
<div class="rb-main-flex-grid-left-column">
	...
	<?php
		...
		$statement = $connection->prepare('SELECT categoryID,categoryTitle FROM blog_categories WHERE categorySlug = :categorySlug');
		$statement->execute(array(':categorySlug' => $id));
		$row = $statement->fetch();
		...
		try {
			...
			$statement = $connection->prepare('SELECT blog_posts.postID FROM blog_posts, blog_post_categories WHERE blog_posts.postID = blog_post_categories.pcPostID AND blog_post_categories.pccategoryID = :categoryID');
			$statement->execute(array(':categoryID' => $row['categoryID']));
