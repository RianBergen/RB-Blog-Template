...
<div class="rb-main-flex-grid-left-column">
	...
	<?php
		try {
			...
			$statement = $connection->prepare('SELECT postID FROM blog_posts WHERE postDate >= :from AND postDate <= :to');
			...
			$statement = $connection->prepare('SELECT postID, postTitle, postSlug, postDescription, postDate, postViewCount, postTags FROM blog_posts WHERE postDate >= :from AND postDate <= :to ORDER BY postID DESC '.$pages->get_limit());
			$statement->execute(array(
				':from' => $from,
				':to' => $to
			));
